let message = `
    ...
    It's really cool that you are going with me.
...
Patrick and Brad both got pretty drunk at this party. Actually, Patrick said that Brad was pretending to be a lot drunker than he really was. They were sitting in the basement with some girl named Heather, and when she left to go to the bathroom, Brad and Patrick were left alone. Patrick said it was uncomfortable and exciting for both of them.
...
let entries = new Map<string, Map<string, number>>();
...
message.split(/<>/).forEach((line: string, i: number, a: string[]) => {
    line.split(/\s/).forEach((word, i, a) => {
        if (!entries.has(word)) {
            let m = new Map();
            m.set(a[i + 1], 1);
            entries.set(word, m);
        } else {
            let m = entries.get(word);
            m.set(a[i + 1], m.has(a[i + 1]) ? m.get(a[i + 1]) + 1 : 1);
            entries.set(word, m);
        }
    })
})

let c = (Math.random() * entries.size)^0;
let word = "";
let lc = 0;
for(let item of entries.keys()){
    if(lc === c){
        word = item;
        break;
    }
...
let chooseWord = (word: string) => {
    if (entries.has(word)) {
        let prob = Math.random();
        let wm = entries.get(word);
        let cc = 0, tc = 0;

        for (let item of wm.values()) {
            tc += item;
        }

        for (let item of wm.entries()) {
            let ip = item[1] / tc;
            let ncc = cc + ip;
            if (prob > cc && prob < ncc) {
                return item[0];
...
let generate = (word: string, count: number) => {
    let s: string = word;
    while(count > 0){
        word = chooseWord(word);
        console.log(`[GENERATING] ${count} - ${word}`);
        let ts = s.split(" ")
        ts.push(word)
        s = ts.join(" ");
        count--;
    }
    return s.replace(/\s<eo>/g, ".")
}

console.log(generate(word, 20));
console.log(entries);